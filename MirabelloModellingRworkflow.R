...
sitepts <- read.csv("data/csvs/mirab-sites.csv", header=T)						#site points
sites_buff <- readOGR(dsn="data/shapefiles/sites_buff.shp", layer="sites_buff")	#site polygons
...
# specify covariates as input in logistic regression model (environmental raster files as im objects)
...
### run models
## function (adapted from logRMCr, Bevan 2011) to produce dataframe with coeffs and intercepts from each spsample data and dummy point ppm
...
# load saved files
load(modabs1, file="r/mods/neomodabs1.RData")
load(modabs2, file="r/mods/neomodabs2.RData")
...
# Second-order modelling using input from first-order PPM Models
...
# stack covs in one obj and mask to study area
...
# calculate average for covariate coefficients from PPM simulations
...
# calc average of the intercept value
int_avg <- mean(intrcpt)

# turn insignif covs to zeros
insig <- c(3,4,5,6,9)			# designate manually according to probability values (abs1probs, abs2probs) at desired level
for (j in 1:length(insig)){
	cov_avg[insig[j]] <- 0}

# log-odds surface based on the averages from simulations
...
# as im object for input in envelope() 
...
# pair correlation function using simulated ppp objs of absence points based on first-order trend surface
#hldsPCFcov <- envelope(as.ppp(coordinates(rdat),W=as.owin(study2)),pcf,simulate=rlmbd,nsim=999,nrank=25)
#save(hldsPCFcov, "r/pcf/neohldscovpcf.RData")

#plotting of pcf results
load(file="r/pcf/neopcfsamp.RData")			# load pcfdat
load(file="r/pcf/neohldscovpcf.RData") 		# load pcf results
